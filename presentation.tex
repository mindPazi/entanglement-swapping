  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}
        ...
        \item Bands: $\pm 1$ std of the per-run distribution, clipped to $T \geq 1$ (high spread at low $\ps$)
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}
        ...
        \item Bands: $\pm 1$ std, clipped to the physical range $[0.25, 1]$
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}
        ...
        \item $F \approx 0.91$ ($N\!=\!1$) to $F \approx 0.41$ ($N\!=\!7$)
        \item Band ($\pm 1$ std) widens with $N$: more uncertainty
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Werner Model for Fidelity}
        ...
        \]
        {\small Exact for ideal BSMs $\Rightarrow$ closed-form prediction of the simulation.}
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Analysis: MC Fidelity vs Werner vs Approximation}
  ...
  \begin{columns}[T]
    \begin{column}{0.45\textwidth}
      \small
      \textbf{Left panel:} per-run \emph{paired} comparison (same generation
      times): Werner prediction and simulation coincide to
      \alert{machine precision} ($\sim 10^{-16}$) --- a closed form, not a fit.
    \end{column}
    \begin{column}{0.45\textwidth}
      \small
      \textbf{Right panel:}
      $F \approx (1 - \pw)^{TN}$ underestimates
      (heuristic from the project plan, consistently pessimistic).
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Analysis: Fidelity Scaling with $N$}
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \vskip 0.8em
      \begin{itemize}
        \item Werner curve \textbf{coincides} with MC at every $N$ (per-run paired, machine precision)
        \item Why exact: depolarized pairs \emph{are} Werner states, and ideal BSMs multiply $w$ exactly
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Main Results}
        ...
        \begin{itemize}\itemsep0.15em
          ...
          \item \textbf{Werner} closed form: \alert{exact} agreement with MC (machine precision)
          \item \textbf{Plan approximation}: heuristic, consistently pessimistic
        \end{itemize}
        \vspace{-0.2em}
      \end{block}
    \end{column}
