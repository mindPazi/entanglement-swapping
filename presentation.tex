...
\begin{frame}{The Protocol: Entanglement Swapping}
  \begin{columns}[c]
    \begin{column}{0.52\textwidth}
      ...
      \vskip 0.5em
      \textbf{Each repeater $R_k$ (a parallel process):}
      \begin{enumerate}
        \item Holds qubits entangled with both neighbours
        \item \textbf{Local BSM} on its two qubits only
        \item Sends the outcome on a \textbf{classical channel}
        \item Remote node applies the \textbf{Pauli correction}
      \end{enumerate}

      \vskip 0.5em
      Repeaters swap \textbf{independently} until Alice and Bob share $\phiplus$.
    \end{column}
...
\begin{frame}{Simulator Architecture}
  \begin{columns}[c]
    ...
    \begin{column}{0.52\textwidth}
      ...
      \vskip 0.2em
      \begin{block}{Discrete-event engine (\texttt{ProtocolZoo})}
        \vspace{-0.3em}
        {\footnotesize Parallel \texttt{@resumable} entities on one \texttt{ConcurrentSim} clock:
        \begin{itemize}\itemsep0em
          \item \texttt{EntanglerProt}: heralded generation per link
          \item \texttt{SwapperProt}: asynchronous local BSM
          \item \texttt{EntanglementTracker}: classical corrections
          \item detector: stops at delivery, measures $F, T$
        \end{itemize}}
        \vspace{-0.3em}
      \end{block}
    \end{column}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}
        ...
        \item $F \approx 0.92$ ($N\!=\!1$) to $F \approx 0.65$ ($N\!=\!7$)
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Werner Model (asynchronous)}
        \small
        Each depolarized qubit survives $(1-\pw)^{\Delta}$ over its own wait
        $\Delta$: a repeater qubit until its swap $\max(T_{i-1},T_i)$, an end
        qubit until $T$.
        \vspace{-0.2em}
        \[
          w_i = (1-\pw)^{\,\Delta^{\mathrm{L}}_i + \Delta^{\mathrm{R}}_i}, \quad
          \Delta^{\mathrm{L}}_i,\Delta^{\mathrm{R}}_i \le T - T_i
        \]
        Ideal BSMs multiply them:
        \vspace{-0.2em}
        \[
          w_{\mathrm{tot}} = \prod_{i=1}^{N+1} w_i, \quad F = \frac{1 + 3\, w_{\mathrm{tot}}}{4}
        \]
        \alert{Per-qubit} waits $\Rightarrow$ higher $F$ than synchronous
        ($\Delta_i = T - T_i$ for all qubits).
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[c]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{itemize}
        ...
        \item \textbf{Synchronous} Werner falls away for $N \ge 2$: the gap grows to $\sim 0.22$ at $N = 7$
